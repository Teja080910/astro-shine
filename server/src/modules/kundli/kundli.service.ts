...
export class KundliService {
  ...
  async create(data: typeof schema.kundliRecords.$inferInsert) {
    ...
    const result = this.astrology.calculateKundli(details);
    const [r] = await this.db.insert(schema.kundliRecords).values({
      ...data,
      chartData: result,
  ...
  async update(id: string, data: Partial<typeof schema.kundliRecords.$inferInsert>) {
    const [r] = await this.db.update(schema.kundliRecords).set(data).where(eq(schema.kundliRecords.id, id)).returning();
    return r;
  }
}
