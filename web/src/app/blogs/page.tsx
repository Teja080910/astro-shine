  ...
  return (
    <AdminLayout>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setEditing({} as Blog)} className="gradient-btn">New Blog</button>
      </div>
      <div className="grid gap-4">
        {data.map(b => (
          <div key={b.id} className="glass-card-solid p-4 flex justify-between items-center">
            ...
            <button onClick={() => { setEditing(b); setForm({ title: b.title, slug: b.slug, content: b.content, status: b.status, tags: (b.tags || []).join(', ') }); }} className="text-primary-light hover:underline text-sm">Edit</button>
          </div>
        ))}
      </div>
